      ...
      <div style={{
        ...
        {messages.length === 0 ? (
          ...
          messages.map(m => {
            ...
            const photo = m.sender_photo
            const avatarStyle = { width: 32, height: 32, borderRadius: '50%', flexShrink: 0, objectFit: 'cover' }
            const fallbackStyle = { ...avatarStyle, background: isSent ? 'linear-gradient(135deg,#6b84e0,#f5a623)' : 'linear-gradient(135deg,#4F6BD9,#F5A623)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: '0.8rem' }
            const avatar = photo
              ? <img src={photo} alt={name} style={avatarStyle} onError={e => { e.currentTarget.style.display='none'; e.currentTarget.nextSibling.style.display='flex' }} />
              : null
            return (
              <div key={m.id} style={{ display: 'flex', justifyContent: isSent ? 'flex-end' : 'flex-start', alignItems: 'flex-end', gap: 8 }}>
                {!isSent && (
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    {avatar}
                    <div style={{ ...fallbackStyle, display: photo ? 'none' : 'flex' }}>{initials}</div>
                  </div>
                )}
                <div style={{ maxWidth: '68%' }}>
                  <div style={{ fontSize: '0.78rem', fontWeight: 600, marginBottom: 3, paddingLeft: isSent ? 0 : 4, paddingRight: isSent ? 4 : 0, opacity: 0.65, textAlign: isSent ? 'right' : 'left' }}>
                    {name}
                  </div>
                  <div style={{
                    padding: '10px 14px',
