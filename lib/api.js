...
/** Refresh cached user from backend and return the latest user object. */
export async function refreshAuthUser() {
  ...
  try {
    ...
    const latestUser = data?.data ?? data
...
export async function apiFetch(path, options = {}) {
  ...
  // If the server says our token is invalid, clear local auth and notify the app
  if (res.status === 401) {
